...
(function($){
	...
	function paginator()
	{
		...
		for (var i = 1; i <= opts.nbpages; i++) {
			if (i == 1 || i == opts.nbpages || opts.pagenbr == i || i == opts.pagenbr-1 || i == opts.pagenbr-2 || i == opts.pagenbr+1 || i == opts.pagenbr+2) {
				...
				if (opts.pagenbr == i) cnt += '<div class="col-lg-1"><input value="'+i+'" id="txtPagitem" class="pagitemtxt form-control" /></div> ';
	...
	{	
		...
		if (typeof gblnumpages != 'undefined') {
			...
			$('#prevbuto-header,#nextbuto-header,#prevbuto-footer,#nextbuto-footer').parent('li').addClass('disabled');
			if (opts.pagenbr <= 1) {
	...
	{
		if (e.length > 0) {
            toggleImgOnOff($('.ui-toolbar-item-filter .fladvfilter > img' ), 'src', 'on');
        }
		else {
            toggleImgOnOff($('.ui-toolbar-item-filter .fladvfilter > img' ), 'src', 'off');
        }
	...
	{
        if($('.ui-toolbar-item-filter .fladvfilter').length > 0){
